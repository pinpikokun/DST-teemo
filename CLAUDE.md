...
- **クロスチェック必須**: 第1ラウンド（並行検証）の結果を集約した後、第2ラウンドとして別のエージェントに「検出された問題が本当にバグか、偽陽性か」を実際のコードを再読して相互検証させること。単に並列で独立チェックするだけでは不十分
- **マルチプレイ挙動の考慮**: サーバー/クライアント間の同期、他プレイヤーへの影響、ホスト/ゲスト間の差異を必ず確認する
- **DST環境固有のエッジケース**: 攻撃可能な壁、海上、ボート上、洞窟（地上↔洞窟の切り替え）での挙動を忘れずに検証する
- **スキップする既知事項（バグ報告不要）**:
  - `_noxiousTraps` リストのセーブ/ロード非永続化: トラップは5分で自然消滅し、スタック回復も遅いため実害なし。意図的なセーブ/ロード繰り返しは遊び方の範疇
  - Noxious Trap の `deployer` 参照のセーブ/ロード非永続化: フォールバック検索で軽減済み、上記と同じ理由でスキップ
